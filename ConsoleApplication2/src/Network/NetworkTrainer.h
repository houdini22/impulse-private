...
#include "../tjungblut-math-cpp/Fmincg.h"
...
public:
	...
	void train(DataSet dataSet) {
		...
		std::cout << network->getLayers()->at(1)->getNeurons()->at(1)->weights->at(0) << std::endl;
		...
		tjmath::DenseVector<double> optimizedParameters = minimizer.minimize(cf, theta, 100, true);
	...
	void train(DataSet dataSet) {
		...
		f.setNetworkTrainer(this);
		f.setDataSet(dataSet);

		TypeVector theta = this->network->getRolledTheta();
		Eigen::VectorXd theta2 = VectorXd::Map(theta.data(), theta.size());

		ConjugatedGradientDescentSolver<Rosenbrock> solver;
		solver.minimize(f, theta2);
	}
	*/
